...
def render() -> None:
    ...
    try:
        ...
        # 🔴 **«0σ 는 가운데» 를 무조건 말하지 않는다** (이슈 #14 · ADR-SC-0017).
        #    축마다는 참이고 총점에서는 거짓이다. 실측 — 축 z 의 일별 중앙값이 0 이
        #    아닌 날은 4축 전부 **0일**이지만, 총점은 균형 **262/266일** · 모멘텀
        #    261일 · 역발상 263일이 0 이 아니다. 이 페이지는 바로 아래에서 총점을
        #    σ 로 보여주므로(«예시 하나를 끝까지») 여기서 가르지 않으면 팀원이
        #    막대의 0 을 가운데로 읽는다.
        st.markdown(
            "같은 날 21개 섹터를 **서로 비교해** 얼마나 튀는지를 잰 값이다. "
            "**축에서는** `+1.0σ` 가 '그 축에서 눈에 띄게 높은 편' 을 뜻한다. "
            "±3.0σ 에서 자른다 — 한 축이 총점을 통째로 지배하지 못하게 "
            "하기 위해서다."
        )
        # 🔴 **총점에 축의 구간 낱말을 붙이지 않는다** (이슈 #17 ① · ADR-SC-0020).
        #    바로 아래 «0σ 는 가운데인가» 와 **같은 모양의 함정**이다 — 축에서는
        #    참이고 총점에서는 거짓이다. 그래서 두 경고를 나란히 둔다.
        st.markdown(
            "🔴 **총점의 σ 는 축의 σ 와 같은 자로 잴 수 없다.** 총점은 네 축 z 의 "
            "**가중평균**이라, 축들이 한꺼번에 같이 움직이지 않는 한 **덜 퍼진다.** "
            "실측(291영업일)으로 축은 0.97~1.27σ 인데 총점은 균형 **0.65** · "
            "모멘텀 **0.78** · 역발상 **0.54** 다. 그래서 **총점에는 '뚜렷이 높다' "
            "같은 말을 붙이지 않는다** — 총점은 **순위로** 읽는다. "
            "«21개 중 3위» 는 언제나 정확한 말이다."
        )
        st.markdown(
            "🔴 **축 하나만 볼 때는 `0σ` 가 정확히 가운데지만, 총점은 다르다.** "
            "축 z 는 축마다 중앙값을 빼서 만들지만 **중앙값은 선형이 아니라서**, "
            "네 축을 가중합한 총점의 가운데는 0 에서 비켜 있다. 한 섹터가 네 축에서 "
            "동시에 가운데인 일이 드물기 때문이다. 그래서 랭킹의 막대는 **가운데에 "
            "점선을 따로 긋는다** — 0 이 아니라 그 점선이 기준이다."
        )
