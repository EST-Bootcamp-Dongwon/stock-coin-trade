...
- 🔴 **앱에 시도 횟수 제한을 걸 수단이 없다.** Streamlit 은 요청 사이에 서버 상태를
  ...
  ⚠️ 2026-09-12 에 여기 *"원장이 Supabase 로 가면 그 수단이 생긴다 — 검증이
    `workspace_append` RPC 안에서 일어나므로 DB 가 횟수를 셀 수 있다"* 고 적었다.
    🔴 **가능성이었지 사실이 아니다**(2026-09-17 · ADR-SC-0011 ⑭). 두 겹으로 틀렸다 —
    ① `workspace_append` 의 분당 60 제한은 passcode 검사 **뒤**에 있고 *삽입된 행*을
    세므로 **틀린 시도는 세지 않는다.** ② 검증은 이제 `workspace_verify_passcode`
    로 옮겨 `append` 안에 있지도 않다.
    🔒 그래서 방어는 여전히 **길이와 해시 비용뿐**이다. 공격자는 scrypt 를 자기
    기계에서 돌리고 결과만 보내므로, 48단어 4개(약 530만)는 수 시간이면 훑린다.
    카운터를 넣지 않은 것은 판단이다 — append-only 원장 옆에 *지울 수 있는* 상태를
    하나 더 만들고, 대회 기간 사용자는 7명이다. 필요해지면 **별도 결정**으로 넣는다.
