...
| 3 | [`decisions/`](decisions/) | ADR-SC-0005~0020 — 되돌리면 안 되는 결정 |
...
| **결정 기록 (ADR)** | ✅ 0005~0020 · 15건 (0009 결번) | [`decisions/`](decisions/) |
| **백로그** | ✅ | [`backlog/`](backlog/) |
...
**테스트**: 루트 `pytest` **1,030건** · `backend` 골든 **23건** (2026-09-23 실측 · 모두 RC=0)
...
flowchart TB
  ...
  subgraph DOCS["docs/ — 결정과 배경"]
    ...
    DE["decisions/ 0005~0020<br/>되돌리면 안 되는 결정"]
...
| [0018](decisions/0018-기여합과-총점의-잔차를-드러낸다.md) | 기여합과 총점의 «잔차» 를 드러낸다 | 09-20 | 🔴 **「합계 = 총점」은 31.5% 에서 거짓** — 등호를 버리고 차이와 **반올림 한도**를 적는다 |
| [0019](decisions/0019-워터폴은-근거-패널-안에-둔다.md) | 워터폴은 «근거 패널» 안에 둔다 | 09-20 | M10 ① — 그림은 **이미 접힌 칸** 안, 축 줄 **위**. 🔒 마지막 막대는 **게시된 총점**에서 끝난다 · 닿는다고 말할 수 없으면 **그리지 않고 왜 없는지 말한다** |
| [0020](decisions/0020-총점은-축의-척도로-말하지-않는다.md) | 총점은 축의 척도로 말하지 않는다 | 09-20 | 이슈 #17 — 🔴 **구간 `0.4/1.0/2.0` 은 축 z 의 문턱인데 총점에 붙어 있었다**(총점 0.538~0.779σ vs 축 0.967~1.274σ). 낱말을 **빼고 순위로 말한다** · 기여 동점은 `lead_axis` 하나가 가른다 · **guard 의 보증 범위**를 ADR-0013 ④ 에 적었다 |
...
| GitLab 이슈 | https://gitlab.com/mygithub05253/stock-coin-trade/-/issues — «화면이 산수에 대해 거짓을 말한다» 3건(**#13 · #14 · #15**)은 **전부 닫혔다**(09-19 · 09-20) |
| [`backlog/확장-요구사항-선별과-산출물.md`](backlog/확장-요구사항-선별과-산출물.md) | 추가 요구 R01~R20 선별 · 평가/가상 포트폴리오 계약 · 산출물 추적표 초안. 구현 미착수 · 첨부 이미지 대조 대기 |
...
| — | 09-20 | 이슈 **#15 · #13** — 가중치 0 축의 «기여 없음» (ADR 0014 ③-1) · 기여합과 총점의 **잔차** (ADR 0018). 1002→**1018건** |
| — | 09-20 | **M10 ① 워터폴** — 근거 패널의 «축별 숫자로 보기» 안에 축 분해 그림 (ADR 0019 · 이슈 #16). 돌연변이 8종 전부 사망 · 1018→**1024건** |
| — | 09-20 | 이슈 **#17** — 총점 척도 혼용 제거 · 기여 동점 규칙 · guard 보증 범위 (ADR 0020). 데이터 9/10→**9/18** 따라잡고 HF 게시(커밋 `077891ad6a83`) |
