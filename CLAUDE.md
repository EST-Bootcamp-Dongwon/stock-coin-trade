...
#### ★ 통째로 복붙되게 낸다 (2026-09-18 사용자 지시)

🔴 **프롬프트 전체를 바깥 코드펜스 하나에 담는다.** 렌더링된 마크다운으로 내면 사용자가
화면에서 긁어 복사해야 하고, 그러다 표·Mermaid 가 깨지거나 일부만 딸려 간다.

- 바깥 펜스는 **백틱 4개**로 연다 — 안에 3개짜리(`bash`·`mermaid`)가 들어가기 때문이다.
  안쪽에 4개짜리가 있으면 바깥을 5개로 늘린다.
- 🔒 **펜스 밖에 내용을 남기지 않는다.** "아래를 그대로 복사해 새 세션에 붙여넣으세요"
  한 줄 외에는 전부 펜스 안이다 — 밖에 남은 문장은 복사되지 않아 다음 세션이 못 본다.
- 여전히 **채팅으로만** 낸다. 별도 파일을 만들지 않는다.
