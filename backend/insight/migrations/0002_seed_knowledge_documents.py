"""RAG 시드 지식 21건 (05 문서 5.1~5.2 · E-06 2장).

v1.0 `python-stock-backend/qdrant_service.py` 의 `SEED` 를 그대로 옮긴 것이다.
**내용은 한 글자도 바꾸지 않았다** — 바꾸면 v1.0 과 검색 결과가 달라져
이관이 제대로 됐는지 확인할 수 없다.

★ **결함 D-8 해소** — v1.0 은 Qdrant 기본값이 `:memory:` 라 컨테이너를 재기동하면
지식이 통째로 사라졌다. 이제 Postgres 테이블에 있으므로 사라지지 않는다.

★ **임베딩은 여기서 만들지 않는다** (05 문서 5.2) ─────────────────────────
fastembed 모델 로드가 수십 초~수 분이라 마이그레이션에서 벡터를 만들면
**배포 파이프라인이 그만큼 멈춘다.** 롤백도 어려워진다.

    embedding = NULL 로 넣고 → python manage.py backfill_embeddings 가 나중에 채운다

벡터가 없는 문서는 코사인 검색에 나오지 않으므로 **비어 있는 동안도 안전하다.**
"검색이 안 된다"가 "틀린 답이 나온다"보다 낫다.
"""

from django.db import migrations

# (title, category, content)
# category 는 v1.0 값을 그대로 쓴다 — technical_analysis / market_structure /
# sector_analysis / investment_strategy / risk_management / fundamental_analysis
SEED_DOCUMENTS = [
    (
        "RSI(상대강도지수) 분석",
        "technical_analysis",
...
TITLES = [row[0] for row in SEED_DOCUMENTS]


def seed_documents(apps, schema_editor):
    """시드 지식 21건을 넣는다. `source="SEED"` · `is_approved=True` · `embedding=NULL`.

    **`is_approved=True` 인 이유** — 우리가 넣은 문서라 검토가 필요 없다.
    사용자가 추가하는 문서(`source="USER"`)만 기본값 `False` 로 격리된다 (결함 D-6).

    ★ Django 관점 — `KnowledgeDocument` 에는 매니저가 둘이다(`objects` · `approved`).
    데이터 마이그레이션의 `apps.get_model()` 이 돌려주는 **과거 모델에는 커스텀 매니저가
    붙지 않는다** — `objects` 만 있고 그것도 소박한 기본 매니저다.
    여기서 `.approved` 를 쓰려 하면 `AttributeError` 가 난다. 지금은 `objects` 만 쓴다.

    재실행 안전성 — `title` 에 UNIQUE 가 없어서 `ignore_conflicts` 로는 중복을 막지
    못한다. 이미 시드가 들어와 있으면 통째로 건너뛴다.
    """
    KnowledgeDocument = apps.get_model("insight", "KnowledgeDocument")

    if KnowledgeDocument.objects.filter(source="SEED").exists():
        return   # 이미 시드가 있다 — 사용자가 편집했을 수 있으므로 손대지 않는다
    ...
    KnowledgeDocument.objects.bulk_create([
        KnowledgeDocument(
            ...
            category=category,
            content=content,
            embedding=None,      # ★ 배치(backfill_embeddings)가 채운다 (05 문서 5.2)
            source="SEED",
            is_approved=True,
        )
...
def unseed_documents(apps, schema_editor):
    """되돌리기 — `source="SEED"` 이면서 제목이 이 목록에 있는 것만 지운다.

    조건을 둘 다 거는 이유는 운영자가 같은 제목으로 손수 추가한 문서
    (`source="USER"`)를 함께 지우지 않기 위해서다.
    """
    KnowledgeDocument = apps.get_model("insight", "KnowledgeDocument")
    KnowledgeDocument.objects.filter(source="SEED", title__in=TITLES).delete()
