"""체결 엔진 **골든 테스트** — 체결 엔진 회귀 고정 장치 (ADR-SC-0002).

⚠️ 2026-09-15 갱신 — `quant-core` 이식 계획은 폐기됐다(`quant-core`·`backtest-service`
모두 개발하지 않기로 해 삭제됨). 이 파일은 더는 이식 대조용이 아니라, 이 저장소 하나의
체결 엔진이 리팩터링을 거쳐도 값이 달라지지 않았는지 고정하는 **회귀 테스트**다.
...
둘은 목적이 다르다. 체결 로직을 고치고 나서 필요한 것은 **결과가 한 글자도
다르지 않다**(또는 바뀌었다면 왜 바뀌었는지 설명할 수 있다)는 증거이고, 그건
의미 서술이 아니라 값 대조로만 얻어진다. `__snapshots__/golden_test.ambr` 가
그 기준값이다.

★★ **DB 를 쓰지 않는다** ──────────────────────────────────────────────────

여기 담긴 함수는 전부 순수 함수다 — ORM 도, 설정 조회도 없다. 그래서 픽스처를
`Orderbook`/`Quote` dataclass 로 직접 만든다. **순수 함수만 다루므로 DB 에 닿을
이유가 없고, 그래야 빠르고 결정적으로 돈다.**
...
#    (이식 계획은 폐기됐으나 체결 가정 명세로는 유효하다).
#    "평균 잔량이 1틱 간격으로 무한히 이어진다"는 **실제 시장보다 유리한 가정**이고,
#    이 골든 값이 그 가정을 고정한다 — 나중에 체결 로직을 손대도 이 값이 그대로면
#    가정이 안 깨진 것이다.
