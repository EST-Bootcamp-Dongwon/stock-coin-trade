"""accounts — 회원 · 계좌 · API 키 · 관심종목 (E-01).

**v2.0 데이터 모델에서 가장 큰 변화가 이 앱에 있다** —
`member.asset` 한 칸이 `Account` 여러 행으로 쪼개진다.

v1.0 은 `member.asset` 하나로 주식·코인·대체자산을 전부 사고팔았다. 그래서
"주식 실력"을 따로 측정할 수 없었고, 계좌 초기화가 코인을 안 지우는 결함(D-5)의
원인이기도 했다.
"""

from django.contrib.auth.models import AbstractUser
from django.db import models

from core.constants import AccountMode, AssetClass
from core.models import TimeStampedModel

# 연습 계좌 시작 자본 1억 — v1.0 INITIAL_ASSET 승계
INITIAL_PRACTICE_CAPITAL = 100_000_000


class Member(AbstractUser):
    """회원.

    v1.0 은 `member` 테이블을 직접 정의하고 bcrypt 해싱·Flask 서명쿠키를 손으로 만들었다.
    `AbstractUser` 를 상속하면 비밀번호 해싱·세션·권한·Admin 연동이 전부 딸려온다.

    Django 관점 — 커스텀 User 모델은 **첫 마이그레이션 전에** 정해야 한다.
    나중에 바꾸려면 기존 FK 를 전부 손봐야 해서 사실상 재구축이다.
    `settings.AUTH_USER_MODEL = "accounts.Member"` 를 처음부터 박아 두었다.

    Django 관점 2 — FastAPI 에서는 `POST /members` 핸들러에서 `bcrypt.hashpw()` 를
    직접 불렀다. Django 는 `Member.objects.create_user(...)` 가 해싱까지 한다.
    **`create()` 를 쓰면 비밀번호가 평문으로 들어간다** — 가장 흔한 실수다.

    **`asset` 컬럼은 없다.** 현금은 전부 `Account.cash` 에 있다.
    **탈퇴는 삭제가 아니다** — `is_active=False` + 개인정보 익명화로 처리한다 (규약 5.1).
    """

    # AbstractUser 의 email 은 유니크가 아니다. v1.0 이 이메일 유니크를 전제했으므로 덮어쓴다
    email = models.EmailField(unique=True, verbose_name="이메일")
    display_name = models.CharField(max_length=30, blank=True, verbose_name="표시명")
    avatar = models.ImageField(
        upload_to="avatars/", max_length=200, null=True, blank=True, verbose_name="프로필 이미지"
    )

    class Meta:
        db_table = "member"
        verbose_name = "회원"
        verbose_name_plural = "회원"

    def __str__(self):
        return f"{self.display_name or self.username}"


class Account(TimeStampedModel):
    """모드별 계좌. **v2.0 데이터 모델의 심장이다.**

    모든 포지션·주문의 소유자가 `member` 에서 `account` 로 바뀐다.

    Django 관점 — FastAPI 에서는 `member.asset` 을 서비스 계층에서 직접 만졌다.
    Django 에서는 `member.accounts.get(mode="PRACTICE_STOCK")` 처럼 역참조 매니저로
    꺼낸다. `related_name="accounts"` 가 그 이름을 만든다.

    생성 시점 —
      · 연습 계좌 3종: **회원 가입 시 자동 생성** (각 1억)
      · 대회 계좌: **참가 승인(APPROVED) 시점** (`Contest.initial_capital`)

    가입 시 자동 생성은 **시그널이 아니라 서비스 함수**에서 한다. 시그널은
    `seed_demo` 커맨드·테스트 픽스처에서도 몰래 발동해 디버깅이 어려워진다.
    """

    member = models.ForeignKey(
        "accounts.Member",
        on_delete=models.PROTECT,          # 회원을 지워도 계좌·거래이력은 남아야 한다
        related_name="accounts",
        verbose_name="회원",
    )
    contest = models.ForeignKey(
        "contests.Contest",                # 문자열 참조 — 순환 임포트를 피한다 (규약 1.1)
        on_delete=models.PROTECT,
        null=True, blank=True,
        related_name="accounts",
        verbose_name="대회",
        help_text="대회 계좌면 값이 있고, 연습 계좌면 비어 있다",
    )
    mode = models.CharField(max_length=20, choices=AccountMode.choices, verbose_name="모드")
    cash = models.BigIntegerField(default=0, verbose_name="현금(원)")
    initial_capital = models.BigIntegerField(
        default=INITIAL_PRACTICE_CAPITAL, verbose_name="시작 자본(원)",
        help_text="수익률 기준선이자 초기화 목표값",
    )
    is_frozen = models.BooleanField(
        default=False, verbose_name="동결 여부", help_text="대회 종료 후 동결 → 주문 불가"
    )
    reset_at = models.DateTimeField(null=True, blank=True, verbose_name="마지막 초기화")

    class Meta:
        db_table = "account"
        verbose_name = "계좌"
        verbose_name_plural = "계좌"
        indexes = [
            models.Index(fields=["member", "mode"], name="account_idx_member_mode"),
            # ★ E-01 4.3 이 명시한 `models.Index(fields=["contest"])` 는 넣지 않는다.
            #   Django 는 **FK 컬럼에 인덱스를 자동으로 만든다** —
            #   `account_contest_id_95b08916` 이 이미 (contest_id) 단일 인덱스다.
            #   같은 인덱스를 두 벌 두면 INSERT·UPDATE 마다 두 번 갱신되고 얻는 게 없다.
            #
            #   Django 관점 — SQLAlchemy 는 FK 에 인덱스를 만들어 주지 않아서
            #   `index=True` 를 손으로 붙이는 게 습관이었다. Django 는 반대로
            #   **자동이 기본**이라, 명세에서 옮겨 적을 때 중복이 생기기 쉽다.
            #   끄고 싶으면 필드에 `db_index=False` 를 준다.
            #
            #   `account_idx_member_mode` 는 복합 인덱스라 해당하지 않는다 —
            #   (member_id, mode) 는 자동 생성분과 컬럼이 다르다.
        ]
        constraints = [
            # ① 연습 계좌 — 회원당 모드별 1개. contest 가 NULL 인 행만 대상으로 한다.
            #    ★ 조건 없이 걸면 SQL 의 "NULL != NULL" 규칙 때문에 무제한으로 생긴다.
            #    unique_together 로는 이게 안 된다 — 조건을 붙일 수 없기 때문이다 (규약 6.1).
            models.UniqueConstraint(
                fields=["member", "mode"],
                condition=models.Q(contest__isnull=True),
                name="account_uniq_practice_mode",
            ),
            # ② 대회 계좌 — 회원당 대회별 1개
            models.UniqueConstraint(
                fields=["member", "contest"],
                condition=models.Q(contest__isnull=False),
                name="account_uniq_contest",
            ),
            # ③ mode 와 contest 가 어긋나지 않게 DB 가 막는다.
            #    "대회 모드인데 대회가 없음" / "연습 모드인데 대회가 붙음" 을 둘 다 차단
            models.CheckConstraint(
                condition=(
                    models.Q(mode=AccountMode.CONTEST, contest__isnull=False)
                    | (~models.Q(mode=AccountMode.CONTEST) & models.Q(contest__isnull=True))
                ),
                name="account_ck_mode_contest_match",
            ),
        ]

    def __str__(self):
        return f"{self.member} / {self.get_mode_display()}"

    @property
    def asset_class(self) -> str:
        """모드가 자산군을 결정한다. 컬럼으로 두지 않는 이유는 규약 8.1 참조."""
        return AccountMode(self.mode).asset_class

    @property
    def is_contest(self) -> bool:
        return self.mode == AccountMode.CONTEST


class ApiKey(TimeStampedModel):
    """Open API 키. 평문은 발급 시 1회만 보여주고 DB 에는 해시만 남긴다 (v1.0 설계 승계).

    ★ **계좌 고정이 필수인 이유** — v1.0 은 회원당 계좌가 하나뿐이라 지정할 게 없었다.
    v2.0 은 계좌가 4개 이상이다. 키에 계좌를 못 박아 요청마다 지정하는 방식을 쓰지 않는다.
    봇이 실수로 대회 계좌에 연습용 주문을 쏘는 사고를 원천 차단한다 (F-13 4.1).

    주의 — 이건 **회원이 우리 서비스를 호출할 때 쓰는 키**다. 우리 서버가 증권사에
    붙을 때 쓰는 자격증명은 `market.ExternalToken` 이고, 그 앱키·시크릿은
    DB 가 아니라 환경변수에 둔다.
    """

    member = models.ForeignKey(
        "accounts.Member", on_delete=models.PROTECT, related_name="api_keys", verbose_name="회원"
    )
    account = models.ForeignKey(
        "accounts.Account",
        on_delete=models.PROTECT,
        related_name="api_keys",
        verbose_name="주문 대상 계좌",
        help_text="이 키로 낸 주문은 항상 이 계좌로 들어간다",
    )
    name = models.CharField(max_length=50, verbose_name="키 이름")
    key_prefix = models.CharField(max_length=16, db_index=True, verbose_name="키 접두")
    key_hash = models.CharField(
        max_length=64, unique=True, verbose_name="키 해시(sha256)",
        help_text="평문은 저장하지 않는다",
    )
    is_active = models.BooleanField(default=True, verbose_name="활성")
    last_used_at = models.DateTimeField(null=True, blank=True, verbose_name="마지막 사용")
    revoked_at = models.DateTimeField(null=True, blank=True, verbose_name="폐기 시각")

    class Meta:
        db_table = "api_key"
        verbose_name = "Open API 키"
        verbose_name_plural = "Open API 키"
        indexes = [models.Index(fields=["member", "is_active"], name="api_key_idx_member_active")]

    def __str__(self):
        return f"{self.name} ({self.key_prefix}…)"


class RateLimitCounter(models.Model):
    """고정 윈도우(1분) 호출 카운터 — 결함 D-9 해소.

    v1.0 은 `dict[api_key_id] → [timestamp...]` 였다. **서버리스에서는 인스턴스마다
    자기 카운터를 갖게 되어 분당 60회 제한이 인스턴스 5개면 300회가 된다.**

    잠금이 필요 없다 — `INSERT ... ON CONFLICT DO UPDATE ... RETURNING` 한 방으로
    증가와 검사를 원자적으로 끝낸다 (F-13 3.2).

    슬라이딩 윈도우를 버린 이유 — 고정 윈도우는 경계에서 최대 2배까지 통과할 수 있다.
    교육용 서비스에서 분당 60이 순간적으로 120이 되는 것은 문제가 되지 않고,
    구현이 SQL 한 줄로 끝난다는 이점이 훨씬 크다.
    """

    api_key = models.ForeignKey(
        "accounts.ApiKey", on_delete=models.CASCADE, related_name="rate_counters"
    )
    window_start = models.DateTimeField(verbose_name="윈도우 시작(분 절삭)")
    count = models.PositiveIntegerField(default=0, verbose_name="호출 수")

    class Meta:
        db_table = "rate_limit_counter"
        verbose_name = "API 유량 카운터"
        verbose_name_plural = "API 유량 카운터"
        constraints = [
            models.UniqueConstraint(
                fields=["api_key", "window_start"], name="rate_limit_uniq_window"
            ),
        ]
        indexes = [
            # cleanup 잡이 지난 윈도우를 지울 때 쓴다
            models.Index(fields=["window_start"], name="rate_limit_idx_window"),
        ]

    def __str__(self):
        return f"{self.api_key_id} {self.window_start:%H:%M} × {self.count}"


class Watchlist(TimeStampedModel):
    """관심종목 — v1.0 의 브라우저 `localStorage` 를 서버로 승격한다.

    기기를 바꾸거나 브라우저 데이터를 지우면 사라지던 문제를 없앤다.

    `asset_class` 를 키에 넣는 이유는 주식 `005930` 과 코인 `KRW-BTC` 가
    한 테이블에 살기 때문이다.
    """

    member = models.ForeignKey(
        "accounts.Member", on_delete=models.CASCADE, related_name="watchlist", verbose_name="회원"
    )
    asset_class = models.CharField(max_length=10, choices=AssetClass.choices)
    symbol = models.CharField(max_length=20)
    sort_order = models.IntegerField(default=0, verbose_name="정렬 순서")

    class Meta:
        db_table = "watchlist"
        verbose_name = "관심종목"
        verbose_name_plural = "관심종목"
        ordering = ["sort_order", "symbol"]
        constraints = [
            models.UniqueConstraint(
                fields=["member", "asset_class", "symbol"], name="watchlist_uniq_symbol"
            ),
        ]

    def __str__(self):
        return f"{self.member} ★ {self.symbol}"
