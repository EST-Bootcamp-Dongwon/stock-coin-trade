...
| `20260917053500` | **읽기 전용 `workspace_verify_passcode`** — `verify` 를 쓰기 경로에서 뗀다 ✅ 2026-09-17 적용 |
| `20260917053600` | **조 생성은 `create_team` 만** — 부분 유니크 인덱스 · `append` 거절 · `create_team` 두 곳 수정 ✅ 2026-09-17 적용 |
...
### 적용은 누가 하나 — 에이전트가 해도 된다 (2026-09-17 사용자 위임)

2026-09-17 까지는 사용자가 SQL 에디터에 붙여 넣었다. 이제 **에이전트가 Supabase MCP 로
직접 적용해도 된다**(사용자 지시). 🔒 **위 GitHub 연동 금지와 모순되지 않는다** — 그쪽이
막는 것은 *커밋이 곧 프로덕션 변경*이 되어 **사이에 사람이 없는** 흐름이고, 이쪽은
사용자가 대화에 있고 적용 전후를 보고받는다.

🔒 그래서 위임과 함께 남는 규율 —

1. **한 번에 하나.** 여러 파일을 한 자리에서 이어 붙이지 않는다
2. **적용 전 사전 확인 쿼리를 먼저 돌린다**(파일 머리주석에 적혀 있다). 되돌릴 수 없는
   것(원장 행 · append-only)이 걸린 마이그레이션은 특히
3. **적용 후 읽기 전용 조회로 실측하고 보고한다.** "적용했다" 가 아니라 *무엇이
   그렇게 됐는지* 를 보고한다 (ADR-SC-0011 ⑭ "적용" 절이 본보기다)
4. 🔴 **되돌릴 수 없는 것에는 먼저 묻는다** — 원장 행을 만들거나 지우는 것,
   `workspace_team_secret` 을 지우는 것(그 조가 영구 사망한다), 트리거 해제
