...
### 4.4 🔒 묻기 전에 리서치하고, 스스로 정한 것도 교차 검증한다 (2026-09-19 신설)

🔴 **사용자 지시** — "저에게 항상 물어봐야 될 것은 질문하고, **그 질문에 대한 내용을
정확하게 리서치한 뒤** 질문하세요. 저의 판단으로 했다고 해도 **리서치를 통해서 교차
검증을 수행 필수**."

이것은 [4.1](#41--작업이-끝나면-문서를-갱신한다--완료-조건이다-2026-09-19-신설)·4.2 처럼
**완료 조건**이다. 두 갈래로 갈린다 —

| 상황 | 무엇을 하나 |
|---|---|
| **물어야 하는 것** (되돌릴 수 없는 것 · 취향이 갈리는 화면 변경 · 범위 판단) | 🔒 **먼저 리서치하고, 선택지마다 실측·대가·기각 사유를 붙여서** 묻는다. 맨손으로 묻지 않는다 |
| **내가 정하는 것** (플랫폼 동작 · 실측으로 답이 나오는 것) | 정하되 🔒 **근거를 교차 검증한다** — 한 출처로 끝내지 않는다 |
| **에이전트·문서가 준 답** | 🔴 **그대로 믿지 않는다.** 핵심 주장을 직접 재현한다 |

🔴 **실증** — 이슈 #14 착수 때 —

- 이슈 본문이 「5,586행 중 274행 = 4.9%」라고 적었으나, 앱은 **언제나 최신일 하루치만**
  그린다(`as_of = data.latest_day(frame)`). 날 기준으로 다시 재니 **159/266일(59.8%)**
  이었다. **행 기준은 노출을 과대평가한 수**였고, 반대로 「최신일은 어긋남 0」이라
  안심할 뻔한 것도 **역발상 프리셋에서는 3개**로 뒤집혔다.
- 적대적 리뷰가 준 🔴 2건을 **그대로 받지 않고 재현했다** — `pd.NA is not None` 이
  `True` 인 것도, `F` 단독 가중치가 **101일 k=20** 인 것도 직접 돌려 확인했다.
- 🔴 **내가 돌린 변이 검사 자체가 두 번 거짓말을 했다.** ① 없는 `--timeout` 옵션 때문에
  pytest 가 `rc=4`(**사용법 오류**)를 냈는데 "죽었다" 로 셌다. ② 돌연변이를
  `"" or "원문"` 으로 만들었는데 파이썬 `or` 가 두 번째를 돌려주어 **화면이 한 글자도
  안 바뀌었다** — 생존처럼 보인 것이 실은 **무연산 돌연변이**였다. **종료 코드의 뜻과
  돌연변이가 실제로 무엇을 바꾸는지까지 확인**해야 검증이 검증이다.

🔒 리서치 없이 던진 질문은 사용자에게 **판단을 떠넘기는 것**이다. 반대로 리서치했다고
혼자 정해도 되는 것은 아니다 — 위 표의 첫 줄은 여전히 묻는다.
🔒 **세션 마무리 복붙 프롬프트에도 이 규칙을 적는다** (사용자 지시).

### 4.4.1 🔒 «취향이 갈리는 화면 변경» 은 **묻지 않는다** — 정하고 **남긴다** (2026-09-20 신설)

🔴 **사용자 지시** — *"리서치해서 교차 검증. 앞으로는 이런 부분 그냥 리서치해서 교차 검증하고
**이슈나 문서로 남기기.**"*

4.4 의 표에서 «물어야 하는 것» 이 **좁아졌다** —

| 전 (2026-09-19) | 후 (2026-09-20) |
|---|---|
| 되돌릴 수 없는 것 · ~~취향이 갈리는 화면 변경~~ · 범위 판단 | 되돌릴 수 없는 것 · 범위 판단 |

화면 문구 · 표시 방향처럼 **실측으로 우열이 갈리는 것**은 묻지 않는다. 대신 셋을 **전부** 한다 —

1. 🔒 **교차 검증한다** — 한 출처로 끝내지 않는다. 실측을 **독립적인 이론값**이나 **다른 방식의 측정**으로
   재현한다. 재현되지 않으면 아직 모르는 것이다.
2. 🔒 **기각한 대안을 전부 적는다** — 무엇을, **어떤 실측 때문에** 기각했는지. 이것이 빠지면 다음 세션이
   같은 길을 다시 걷는다.
3. 🔒 **ADR 또는 이슈로 남긴다** (4.1 · 4.2). 🔴 **채팅에만 남기지 않는다** — 다음 세션은 그것을 못 본다.

🔒 **여전히 묻는 것** — ① **되돌릴 수 없는 것**(원격 쓰기 · 이력 재작성 · 원장·시크릿 행 삭제 · 저장소
공개 범위 · 계정) ② **범위 판단**(이 세션에서 어디까지 할 것인가).

**실증** (2026-09-20 · 이슈 #17 ① · [ADR-SC-0020](docs/decisions/0020-총점은-축의-척도로-말하지-않는다.md)) —
선택지 (A)(B)(C)(D) 를 놓고 묻는 대신 **교차 검증 셋**이 답을 갈랐다:

| 검증 | 결과 | 무엇을 갈랐나 |
|---|---|---|
| 공분산 이론값 vs 실측 | 오차 **0.00000** | 원인 진단이 **확정**됐다 |
| 배율 추정치의 흔들림 | 19~38% (contrarian 0.204~1.302) | (A)·(D) **기각** |
| 축 z 도 std ≈ 1 이 아님 | 0.576~1.875 | 밴드가 «표준편차 배수» 가 아니라는 **재해석** |

🔴 리서치 없이 던지는 질문이 판단을 떠넘기는 것이라면, **리서치하고도 묻는 것은 시간을 떠넘기는 것**이다.
...
- **passcode 검증은 화면이 아니라 저장소가 한다** ★ (2026-09-12 · ADR-SC-0011 ⑨⑩⑪⑫ ·
  ...
  - 🔒 클라이언트는 **`requests` + PostgREST**. `supabase-py`·`psycopg` 를 넣지 않는다
    (루트 `requirements.txt` 는 5줄이고 Streamlit Cloud 메모리를 직접 깎는다)
